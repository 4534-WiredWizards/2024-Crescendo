...
import frc.robot.commands.FocusShoot;
...
public class RobotContainer {
  ...
  // Photon vision intilization
  public final PhotonCamera camera = new PhotonCamera("orangePyCam");
  ...
  private void configureButtonBindings() {
    ...
    // ----------------------- SHOOTER COMMANDS ---------------------------------
    // Select Button - AUTO SHOOT - Face the speaker, calculate arm angle, spin up
    // shooter, run intake, take shot
    // Highly recommend using this button for shooting over PID close speaker
    // shooting
    ...
    new JoystickButton(operatorJoystick, InputDevices.btn_start)
      .whileTrue(
        new ParallelCommandGroup(
          new RunShooter(shooter, intake, () -> 0.3, false, false, false),
          new RunIntake(intake, true, .75, false)
        )
      );

    // Make a new command similar to the above but it requires the button be held
    // down to run
    // new JoystickButton(operatorJoystick, InputDevices.btn_start)
    //         .whileTrue(
    //                 new FocusShoot(
    ...
    // Bumper Left & Right (Left- Move arm twoards the intake, Right- Move arm away
    // from intake)
